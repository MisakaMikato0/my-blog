...
草稿、密码文章以及设置了 `wikiExclude: true` 的文章不会进入 Wiki，但仍可正常生成站内文章页。文章更新后重新构建即可同步机器入口。
...
构建产物 `dist/` 可直接部署到 Cloudflare Pages、Vercel、Netlify、Nginx 等静态托管平台。需要动态相册、GitHub 贡献数据或 AI 解卦接口时，继续使用项目的 Cloudflare Pages Worker 配置。
