...
>
	{#if dragOver}
		...
		<p class="admin-uploadzone__title">{i18n(I18nKey.dynamicDropHere)}</p>
	{:else if uploading}
		<Icon name="svg-spinners:ring-resize" class="admin-uploadzone__icon" />
		<p class="admin-uploadzone__title">{i18n(I18nKey.dynamicUploading)}</p>
	{:else}
		<Icon name="material-symbols:add-photo-alternate-rounded" class="admin-uploadzone__icon" />
		<p class="admin-uploadzone__title">{i18n(I18nKey.dynamicUpload)}</p>
		<p class="admin-uploadzone__hint">{i18n(I18nKey.dynamicUploadHint)}</p>
	{/if}
	<input
		bind:this={input}
		type="file"
