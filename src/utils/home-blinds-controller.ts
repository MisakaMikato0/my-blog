...
export type HomeBlindsRuntimeConfig = {
	reveal: {
...
export type BootHomeBlindsOptions = {
	root?: HTMLElement;
...
export function getHomeBlindsBackdropVisibility(
	phase: "reveal" | "shrink" | "scenes" | "done",
...
export function getHomeBlindsBackdropDisplay(
	phase: "reveal" | "shrink" | "scenes" | "done",
	sceneProgress: number,
) {
	return getHomeBlindsBackdropVisibility(phase, sceneProgress)
		? "block"
		: "none";
}

/**
 * Embedded HomeBlinds starts its scenes pin after a quarter of the scene
 * distance, then remains pinned for the configured scene distance.
 * Keep this calculation shared with the outer display timeline so the
 * portfolio cannot begin before the scenes have actually completed.
 */
export function getHomeBlindsEmbeddedCompletionDistance(
	scrollDistance: number,
) {
	const distance = Math.max(
		1,
		Number.isFinite(Number(scrollDistance)) ? Number(scrollDistance) : 0,
	);
	return Math.max(1, distance * 0.25) + distance;
}
...
export function normalizeHomeBlindsRuntimeConfig(
	input: unknown,
): HomeBlindsRuntimeConfig {
	const value = isRecord(input) ? input : {};
	const reveal = isRecord(value.reveal) ? value.reveal : {};
	const headline = isRecord(reveal.headline) ? reveal.headline : {};
	const scenes = isRecord(value.scenes) ? value.scenes : {};
	return {
		reveal: {
			foregroundOpacity: clamp(
				Number(reveal.foregroundOpacity ?? 0.5) || 0.5,
				0,
				1,
			),
			pointerTravel: Math.max(0, Number(reveal.pointerTravel ?? 28) || 28),
			headline: {
				enterDuration: Math.max(
					0.1,
					Number(headline.enterDuration ?? 0.5) || 0.5,
				),
				messageHold: Math.max(0.1, Number(headline.messageHold ?? 2.6) || 2.6),
				messageFlipDuration: Math.max(
					0.1,
					Number(headline.messageFlipDuration ?? 0.62) || 0.62,
				),
			},
		},
		scenes: {
			scrollDistance: Math.max(
				1,
				Number(scenes.scrollDistance ?? 3400) || 3400,
			),
			sceneCount: clamp(Math.round(Number(scenes.sceneCount ?? 1) || 1), 1, 5),
			standCount: clamp(Math.round(Number(scenes.standCount ?? 0) || 0), 0, 4),
			cycleDuration: Math.max(4, Number(scenes.cycleDuration ?? 26) || 26),
		},
	};
}

function selectRequired<T extends Element>(root: Element, selector: string): T {
...
function parseRuntimeConfig(root: HTMLElement): HomeBlindsRuntimeConfig {
	...
	return normalizeHomeBlindsRuntimeConfig(
...
function setupScenes(context: SetupContext) {
	...
	function applyPhase() {
		// 嵌入 HomeDisplayLayer 时，独立的 root trigger 可能晚于 scenes trigger
		// 回调一帧更新；不能让 rootInView 的初始 false 把正在横向播放的舞台误隐藏。
		const stageVisible = embedded
			? phase !== "done"
			: rootInView && phase !== "done";
		gsap.set(stage, { autoAlpha: stageVisible ? 1 : 0 });
		const sceneProgress = pinTrigger?.progress ?? 0;
		const backdropVisible = getHomeBlindsBackdropVisibility(
			phase,
			sceneProgress,
		);
		gsap.set(stageBackground, {
			autoAlpha: backdropVisible ? 1 : 0,
			display: getHomeBlindsBackdropDisplay(phase, sceneProgress),
		});
		gsap.set([stageForegroundWindow, stageHeadlineWindow], {
	...
	function syncStage() {
		...
		horizontalEnabled = phase === "scenes" && progress < 0.999;
	...
	const shrinkTimeline = gsap.timeline({
		scrollTrigger: {
			...
			trigger: embedded ? triggerElement : section,
			start: embedded ? "top top" : "top 66.666%",
			end: embedded ? `+=${Math.max(1, scrollDistance * 0.25)}` : "top top",
			scrub: 0.4,
	...
	pinTrigger = ScrollTrigger.create({
		...
		start: embedded
			? `top+=${Math.max(1, scrollDistance * 0.25)} top`
			: "top top",
		end: () =>
			embedded
				? `+=${scrollDistance}`
		...
		...(embedded ? {} : { pin: true, pinSpacing: true }),
		anticipatePin: 1,
		invalidateOnRefresh: true,
		onToggle: syncStage,
		onUpdate: (self) => {
	...
	const rootTrigger = ScrollTrigger.create({
		...
		trigger: embedded ? triggerElement : root,
		start: embedded ? "top top" : "top bottom",
		end: embedded ? () => `+=${scrollDistance}` : "bottom top",
		invalidateOnRefresh: true,
		onToggle: (self) => {
	...
	const revealTimeline = gsap.timeline({
		scrollTrigger: {
			...
			start: embedded ? "top top" : "top 66.666%",
				...
				? `+=${Math.max(1, scrollDistance * 0.25)}`
				: "bottom 66.666%",
			scrub: 0.35,
			invalidateOnRefresh: true,
		},
...
function setupHeadline(context: SetupContext) {
	...
	const trigger = ScrollTrigger.create({
		...
		trigger: embedded ? triggerElement : section,
		start: embedded ? "top top" : "top 66.666%",
		end: embedded ? `+=${Math.max(1, scrollDistance * 0.25)}` : "bottom top",
		invalidateOnRefresh: true,
		onToggle: (self) => {
...
async function initializeHomeBlinds(
	root: HTMLElement,
	viewport: HTMLElement,
	generation: number,
) {
	const config = parseRuntimeConfig(root);
	const embedded = root.dataset.homeBlindsEmbedded === "true";
	...
	const context: SetupContext = {
		...
		embedded,
		scrollDistance: Math.max(1, config.scenes.scrollDistance),
		gsap,
		ScrollTrigger,
		config,
...
export function bootHomeBlinds(options: BootHomeBlindsOptions = {}) {
	bindMediaWatchers();
	const nextRoot = options.root ?? document.getElementById("home-blinds");
	const nextViewport =
		options.viewport ??
		nextRoot?.closest<HTMLElement>("[data-display-viewport]") ??
		nextRoot;
	...
	if (!nextRoot || !nextViewport || !canInitialize(nextRoot)) {
		if (nextRoot) nextRoot.dataset.homeBlindsReady = "inactive";
		return;
	}

	const generation = bootGeneration;
	void initializeHomeBlinds(nextRoot, nextViewport, generation)
		.then((cleanup) => {
			if (!cleanup) return;
